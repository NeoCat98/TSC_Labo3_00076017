Matrix createLocalK(int element,mesh &m){
    ...
    float e = m.getParameter(E_CONDUCTIVITY),a = m.getParameter(A_CONDUCTIVITY),
    l = m.getNode(element+1).getX() - m.getNode(element).getX(), k=e*a*0.5;
...
Vector createLocalb(int element,mesh &m){
    Vector b;
    float j = m.getParameter(HEAT_SOURCE)*-1,
